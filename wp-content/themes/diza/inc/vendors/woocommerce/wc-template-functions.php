...
if ( !function_exists('diza_tbay_woocommerce_breadcrumb_defaults') ) {
    function diza_tbay_woocommerce_breadcrumb_defaults( $args ) {
        ...
        if( $breadcrumbs_layout == 'image' ) {
            ...
            if( is_single() ) {
                $title = '<h1 class="page-title">'. esc_html__('Product', 'diza') .'</h1>';
            } else{
                $title = '<h1 class="page-title">'. esc_html__('Shop', 'diza') .'</h1>';
            } 
        } else {
...
if ( ! function_exists( 'diza_woocommerce_cart_item_name' ) ) {
    function diza_woocommerce_cart_item_name( $name, $cart_item, $cart_item_key ) { 
        ...
        $thumbnail      = $_product->get_image('diza_photo_reviews_thumbnail_image');
...
/** Ajax Elementor Addon diza Product Tabs **/
if ( ! function_exists( 'diza_get_products_tab_ajax' ) ) {
	function diza_get_products_tab_ajax() {
		if ( ! empty( $_POST['atts'] ) ) {
            
			$atts                   = diza_clean( $_POST['atts'] );
            $product_type           = diza_clean( $_POST['value'] );
            $atts['product_type']   = $product_type; 

			$data = diza_elementor_products_ajax_template( $atts );
			echo json_encode( $data );
			die();
		} 
	}
	add_action( 'wp_ajax_diza_get_products_tab_shortcode', 'diza_get_products_tab_ajax' );
	add_action( 'wp_ajax_nopriv_diza_get_products_tab_shortcode', 'diza_get_products_tab_ajax' );
}

/** Ajax Elementor Addon Product Categories Tabs **/
if ( ! function_exists( 'diza_get_products_categories_tab_shortcode' ) ) {
	function diza_get_products_categories_tab_shortcode() {
		if ( ! empty( $_POST['atts'] ) ) {
            
			$atts               = diza_clean( $_POST['atts'] );  
            $categories         = diza_clean( $_POST['value'] );
	...
	add_action( 'wp_ajax_diza_get_products_categories_tab_shortcode', 'diza_get_products_categories_tab_shortcode' );
	add_action( 'wp_ajax_nopriv_diza_get_products_categories_tab_shortcode', 'diza_get_products_categories_tab_shortcode' );
}


if ( ! function_exists( 'diza_get_query_products' ) ) {
    function diza_get_query_products($categories = array(), $cat_operator = 'IN', $product_type = 'newest', $limit = '', $orderby = '', $order = '')
    {
        $atts = [
            'limit' => $limit,
            'orderby' => $orderby,
        ...
        $args = diza_get_attribute_query_product_type($args, $product_type);
        return new WP_Query($args);
    }
}

if ( ! function_exists( 'diza_get_attribute_query_product_type' ) ) {
    function diza_get_attribute_query_product_type($args, $product_type)
    {
        global $woocommerce;
...
if ( ! function_exists( 'diza_elementor_products_ajax_template' ) ) {
	function diza_elementor_products_ajax_template( $settings ) {
        $cat_operator = 'IN';
        
        extract($settings); 
   
        $loop = diza_get_query_products($categories, $cat_operator, $product_type, $limit, $orderby, $order);
