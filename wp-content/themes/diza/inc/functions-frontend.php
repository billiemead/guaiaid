...
if ( ! function_exists( 'diza_tbay_paging_nav' ) ) {
	function diza_tbay_paging_nav() {
		...
		<nav class="navigation paging-navigation">
			<h1 class="screen-reader-text hidden"><?php esc_html_e( 'Posts navigation', 'diza' ); ?></h1>
...
if ( ! function_exists( 'diza_tbay_post_nav' ) ) {
	function diza_tbay_post_nav() {
		...
		<nav class="navigation post-navigation">
			<h3 class="screen-reader-text"><?php esc_html_e( 'Post navigation', 'diza' ); ?></h3>
...
if (!function_exists('diza_get_header_id')) {
    function diza_get_header_id()
    {
...
if (!function_exists('diza_get_display_header_builder') ) {
	function diza_get_display_header_builder() {
		$id = diza_get_header_id();
		
		return  diza_get_html_custom_post($id);
	} 
}

if (!function_exists('diza_get_footer_id')) {
    function diza_get_footer_id()
    {
...
if (!function_exists('diza_get_display_footer_builder') ) {
	function diza_get_display_footer_builder() {
		$id = diza_get_footer_id();
...
if (!function_exists('diza_tbay_display_footer_builder') ) {
	function diza_tbay_display_footer_builder() {
		echo diza_get_display_footer_builder();
	}
}
...
if( ! function_exists( 'diza_get_html_custom_post' ) ) {
	function diza_get_html_custom_post($id) {
        if( is_null($id) || empty($id) ) return;
        
        $post = get_post( $id );

        if ( diza_is_elementor_activated() && Elementor\Plugin::instance()->documents->get( $id )->is_built_with_elementor() ) {
            return Elementor\Plugin::instance()->frontend->get_builder_content_for_display($id, diza_get_elementor_css_print_method());
        } else {
...
if( ! function_exists('diza_load_html_dropdowns_action') ) {
	function diza_load_html_dropdowns_action() {
		$response = array(
			'status' => 'error',
		...
		if( isset( $_POST['ids'] ) && is_array( $_POST['ids'] ) ) {
			$ids = diza_clean( $_POST['ids'] );
			foreach ($ids as $id) {
				$id = (int) $id;
				$content = diza_get_html_custom_post($id);
				if( ! $content ) continue;
	...
	add_action( 'wp_ajax_diza_load_html_dropdowns', 'diza_load_html_dropdowns_action' );
	add_action( 'wp_ajax_nopriv_diza_load_html_dropdowns', 'diza_load_html_dropdowns_action' );
}

if( ! function_exists('diza_load_html_click_action') ) {
	function diza_load_html_click_action() {
		$response = array(
			'status' => 'error',
			'message' => 'Can\'t load HTML blocks with AJAX',
			'data' => array(),
		...
		if( ! empty( $_POST['slug'] ) ) {
			$slug 			= diza_clean( $_POST['slug'] );
			$layout 		= diza_clean( $_POST['layout'] );
			...
			$args['menu_class'] = diza_nav_menu_get_menu_class($layout);
	...
	add_action( 'wp_ajax_diza_load_html_click', 'diza_load_html_click_action' );
	add_action( 'wp_ajax_nopriv_diza_load_html_click', 'diza_load_html_click_action' );
}


if ( ! function_exists( 'diza_get_elementor_post_scripts' ) ) {
	function diza_get_elementor_post_scripts() {
		if( !diza_is_elementor_activated() ) return;
		...
		if ( class_exists( '\Elementor\Core\Files\CSS\Post' ) ) {
			$css_file = new \Elementor\Core\Files\CSS\Post( diza_get_header_id() );
		} elseif ( class_exists( '\Elementor\Post_CSS_File' ) ) {
			$css_file = new \Elementor\Post_CSS_File( diza_get_header_id() );
		}
	
		$css_file->enqueue();

		if ( class_exists( '\Elementor\Core\Files\CSS\Post' ) ) {
			$css_file = new \Elementor\Core\Files\CSS\Post( diza_get_footer_id() );
		} elseif ( class_exists( '\Elementor\Post_CSS_File' ) ) {
			$css_file = new \Elementor\Post_CSS_File( diza_get_footer_id() );
		}
