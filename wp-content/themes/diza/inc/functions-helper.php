...
if (!function_exists('diza_elements_ajax_tabs')) {
    function diza_elements_ajax_tabs()
    { 
        $array = [
            'product-categories-tabs',  
            'product-tabs',
...
if(!function_exists('diza_localize_translate')) {
	function diza_localize_translate() { 
		...
		$diza_hash_transient = get_transient( 'diza-hash-time' );
		if ( false === $diza_hash_transient ) {
			$diza_hash_transient = time();
			set_transient( 'diza-hash-time', $diza_hash_transient );
		}

	    $config = array(
			'storage_key'  		=> apply_filters( 'diza_storage_key', 'diza_' . md5( get_current_blog_id() . '_' . get_site_url( get_current_blog_id(), '/' ) . get_template() . $diza_hash_transient ) ),
	        ...
	        'elements_ready'  => array(
				...
				'ajax_tabs'           => diza_elements_ajax_tabs(),
	            'countdowntimer'      => diza_elements_ready_countdown_timer(),
	        ) 
	    );

		if( diza_is_elementor_activated() ) {
			$config['combined_css'] = diza_get_elementor_css_print_method();
		}
	    ...
	    if( defined('DIZA_WOOCOMMERCE_ACTIVED') && DIZA_WOOCOMMERCE_ACTIVED ) {  
	        ...
	        $config['i18n_checkout']        =  esc_html__('Checkout', 'diza');
			...
			$config['images_mode']        =  apply_filters('diza_woo_display_image_mode', 10, 2);
			$config['single_product']     = apply_filters('diza_active_single_product', is_product(), 10, 2);   
	    }
 
	    return apply_filters('diza_localize_translate', $config);
	}
}
...
if( !function_exists('diza_rocket_lazyload_exclude_class') ) {
	function diza_rocket_lazyload_exclude_class( $attributes ) {
	...
	add_filter( 'rocket_lazyload_excluded_attributes', 'diza_rocket_lazyload_exclude_class' );
}

if ( ! function_exists( 'diza_clean' ) ) {
	function diza_clean( $var ) { 
		if ( is_array( $var ) ) {
			return array_map( 'diza_clean', $var );
		} else {
...
if ( ! function_exists( 'diza_clear_transient' ) ) {
	function diza_clear_transient() {
		delete_transient( 'diza-hash-time' );
	} 
	add_action( 'wp_update_nav_menu_item', 'diza_clear_transient', 11, 1 );
}

if (! function_exists('diza_nav_menu_get_menu_class')) {
    function diza_nav_menu_get_menu_class($layout)
    {
...
if (! function_exists('diza_get_transliterate')) {
    function diza_get_transliterate($title) {
