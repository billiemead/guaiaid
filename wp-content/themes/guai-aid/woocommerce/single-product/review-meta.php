...
<?php } else { ?>
<h5 class="woocommerce-review__author m-0">
    <?php 
	    comment_author();
	    if ( 'yes' === get_option( 'woocommerce_review_rating_verification_label' ) && $verified ) {
				echo '<span class="woocommerce-review__verified verified ml-2">(' . esc_attr__( 'verified owner', 'woocommerce' ) . ')</span> ';
		}
	?>
</h5>
<time class="woocommerce-review__published-date" datetime="<?php echo esc_attr( get_comment_date( 'c' ) ); ?>">
    <?php echo esc_html( get_comment_date( wc_date_format() ) ); ?></time>
<?php
}